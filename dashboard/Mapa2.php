...
$consulta3 = mysqli_query ($conexion,"SELECT * FROM mesas WHERE reservada=true;")
                or die("Error en la consulta: ".mysql_error());
...
$numrs=mysqli_num_rows($consulta3);

   if ($numrs==0)
   $boton = "<input type='submit' class='btn btn-primary' value='Vaciar' disabled> ";
 else
  $boton = "<input type='submit' class='btn btn-primary' value='Vaciar'> ";
...
$consulta8 = mysqli_query ($conexion,"SELECT Nombre FROM `reservaciones` WHERE Mesa='$Mesa' and Inactivo=0;")
                or die("Error en la consulta: ".mysql_error());
while($fila8 = mysqli_fetch_array($consulta8)){
$NombreCliente = $fila8["Nombre"];
}

  $sql="UPDATE mesas SET  reservada=0 WHERE id='$Mesa'";
// SET ContactName = 'Alfred Schmidt', City= 'Frankfurt' WHERE CustomerID = 1;
  ...
  $sql="UPDATE reservaciones SET Inactivo=1 WHERE nombre='$NombreCliente'";
